...
- имя кластера: mycluster
- класс хоста: c4a-c2-m4, диск network-ssd 10Gb; 
- хосты: два хоста в двух разных зонах доступности, указал необходимость публичного доступа для каждого (публичный IP адрес); 
- учётная запись для пользователя: username и имя базы: db1.
...
Использовал docker compose: cкачал SSL-сертификат и подключился к кластеру с помощью утилиты psql,
...
Создал таблицу и вставил две строки:
```
...
host передал имя хоста-реплики. Роли хостов посмотрел на соответствующей вкладке UI консоли.
